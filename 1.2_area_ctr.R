...
ctr_billing <- ctr_billing %>%
  dplyr::rename(ecode = e_code,
         ...
         areabandd_2324 = x17_average_band_d_2_adult_equivalent_council_tax_for_area_of_the_billing_authority_including_both_local_and_major_precepts_lines_8_16_a_16_b_16_c_16_d, 
  )
...
ctr_billing <- ctr_billing %>% 
  select(ecode:class, tstb_2223:tstb_2324, areabandd_2324)
...
ctr <- ctr %>% 
  ...
  mutate(areabandd_2324 = ifelse(ctr$areabandd_2324 == "[z]", 0, areabandd_2324))

ctr <- ctr %>% 
  mutate(tstb_2223 = as.numeric(tstb_2223)) %>% 
  mutate(tstb_2324 = as.numeric(tstb_2324)) %>% 
  mutate(areabandd_2324 = as.numeric(areabandd_2324))
write.csv(ctr, "D:\\Users\\emily.keenan\\OneDrive - MHCLG\\Documents\\GitHub\\council-tax\\output\\areabandd_ctr.csv")
